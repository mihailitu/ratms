...
bool DatabaseManager::initialize() {
  ...
  if (rc != SQLITE_OK) {
    ...
    LOG_ERROR(LogComponent::Database, "Failed to open database: {}",
              last_error_);
    return false;
  }
...
bool DatabaseManager::runMigrations(const std::string &migrations_dir) {
  ...
  LOG_INFO(LogComponent::Database, "Running database migration: {}",
           migration_file_001);
  ...
  std::string migration_file_002 =
      migrations_dir + "/002_optimization_runs.sql";
  LOG_INFO(LogComponent::Database, "Running database migration: {}",
           migration_file_002);
  ...
  std::string migration_file_006 =
      migrations_dir + "/006_profile_traffic_lights.sql";
  LOG_INFO(LogComponent::Database, "Running database migration: {}",
           migration_file_006);
  ...
  std::string migration_file_007 =
      migrations_dir + "/007_traffic_feed_entries.sql";
  LOG_INFO(LogComponent::Database, "Running database migration: {}",
           migration_file_007);
  ...
  LOG_INFO(LogComponent::Database,
...
int DatabaseManager::createSimulation(const std::string &name,
                                      const std::string &description,
                                      int network_id,
                                      const std::string &config_json) {
  std::string sql =
      "INSERT INTO simulations (name, description, network_id, status, "
  ...
  if (rc != SQLITE_OK) {
    last_error_ = sqlite3_errmsg(db_);
    LOG_ERROR(LogComponent::Database, "Failed to prepare statement: {}",
              last_error_);
    return -1;
  }
  ...
  if (rc != SQLITE_DONE) {
    last_error_ = sqlite3_errmsg(db_);
    LOG_ERROR(LogComponent::Database, "Failed to insert simulation: {}",
              last_error_);
    return -1;
  }
...
bool DatabaseManager::updateSimulationStatus(int sim_id,
  ...
  LOG_DEBUG(LogComponent::Database, "Updated simulation {} status to: {}",
            sim_id, status);
  return true;
}

bool DatabaseManager::completeSimulation(int sim_id, long end_time,
                                         double duration) {
  std::string sql =
      "UPDATE simulations SET status = 'completed', end_time = ?, "
...
DatabaseManager::SimulationRecord DatabaseManager::getSimulation(int sim_id) {
  std::string sql =
      "SELECT id, name, description, network_id, status, start_time, "
...
std::vector<DatabaseManager::SimulationRecord>
DatabaseManager::getAllSimulations() {
...
// Metrics operations
bool DatabaseManager::insertMetric(int simulation_id, double timestamp,
                                   const std::string &metric_type, int road_id,
                                   double value, const std::string &unit,
                                   const std::string &metadata_json) {
  std::string sql =
      "INSERT INTO metrics (simulation_id, timestamp, metric_type, road_id, "
...
std::vector<DatabaseManager::MetricRecord>
DatabaseManager::getMetrics(int simulation_id) {
  std::vector<MetricRecord> results;
  std::string sql =
      "SELECT id, simulation_id, timestamp, metric_type, road_id, value, "
...
std::vector<DatabaseManager::MetricRecord>
DatabaseManager::getMetricsByType(int simulation_id,
                                  const std::string &metric_type) {
  std::vector<MetricRecord> results;
  std::string sql =
      "SELECT id, simulation_id, timestamp, metric_type, road_id, value, "
      "unit, metadata_json FROM metrics WHERE simulation_id = ? AND "
      "metric_type = ? "
      "ORDER BY timestamp ASC";
...
// Network operations
int DatabaseManager::createNetwork(const std::string &name,
                                   const std::string &description,
                                   int road_count, int intersection_count,
                                   const std::string &config_json) {
  std::string sql = "INSERT INTO networks (name, description, road_count, "
                    "intersection_count, "
                    "config_json) VALUES (?, ?, ?, ?, ?)";
...
DatabaseManager::NetworkRecord DatabaseManager::getNetwork(int network_id) {
  std::string sql =
      "SELECT id, name, description, road_count, intersection_count, "
...
int DatabaseManager::createOptimizationRun(
    const OptimizationRunRecord &record) {
  std::string sql =
      "INSERT INTO optimization_runs (network_id, status, population_size, "
  ...
  if (rc != SQLITE_OK) {
    last_error_ = sqlite3_errmsg(db_);
    LOG_ERROR(LogComponent::Database,
              "Failed to prepare optimization run insert: {}", last_error_);
    return -1;
  }
  ...
  if (rc != SQLITE_DONE) {
    last_error_ = sqlite3_errmsg(db_);
    LOG_ERROR(LogComponent::Database, "Failed to insert optimization run: {}",
              last_error_);
    return -1;
  }

  int run_id = sqlite3_last_insert_rowid(db_);
  LOG_DEBUG(LogComponent::Database, "Created optimization run with ID: {}",
            run_id);
  return run_id;
}

bool DatabaseManager::updateOptimizationRunStatus(int run_id,
                                                  const std::string &status) {
  ...
  LOG_DEBUG(LogComponent::Database, "Updated optimization run {} status to: {}",
            run_id, status);
  return true;
}

bool DatabaseManager::completeOptimizationRun(int run_id, long completed_at,
                                              long duration_seconds,
                                              double baseline_fitness,
                                              double best_fitness,
                                              double improvement_percent) {
  std::string sql =
      "UPDATE optimization_runs SET status = 'completed', completed_at = ?, "
...
DatabaseManager::OptimizationRunRecord
DatabaseManager::getOptimizationRun(int run_id) {
  std::string sql =
      "SELECT id, network_id, status, population_size, generations, "
  ...
  if (sqlite3_step(stmt) == SQLITE_ROW) {
    ...
    const char *created_by_text =
        reinterpret_cast<const char *>(sqlite3_column_text(stmt, 20));
    if (created_by_text)
      record.created_by = created_by_text;

    const char *notes_text =
...
std::vector<DatabaseManager::OptimizationRunRecord>
DatabaseManager::getAllOptimizationRuns() {
...
std::vector<DatabaseManager::OptimizationRunRecord>
DatabaseManager::getOptimizationRunsByStatus(const std::string &status) {
  std::vector<OptimizationRunRecord> results;
  std::string sql =
      "SELECT id, network_id, status, population_size, generations, "
      "mutation_rate, crossover_rate, elitism_rate, min_green_time, "
...
bool DatabaseManager::insertOptimizationGeneration(
    const OptimizationGenerationRecord &record) {
  std::string sql = "INSERT INTO optimization_generations "
...
bool DatabaseManager::insertOptimizationGenerationsBatch(
    const std::vector<OptimizationGenerationRecord> &records) {
  // Start transaction
...
std::vector<DatabaseManager::OptimizationGenerationRecord>
DatabaseManager::getOptimizationGenerations(int run_id) {
  std::vector<OptimizationGenerationRecord> results;
  std::string sql =
      "SELECT id, optimization_run_id, generation_number, best_fitness, "
...
int DatabaseManager::insertOptimizationSolution(
    const OptimizationSolutionRecord &record) {
  std::string sql =
      "INSERT INTO optimization_solutions (optimization_run_id, "
      "is_best_solution, "
  ...
  if (rc != SQLITE_OK) {
    last_error_ = sqlite3_errmsg(db_);
    LOG_ERROR(LogComponent::Database, "Failed to prepare solution insert: {}",
              last_error_);
    return -1;
  }
  ...
  sqlite3_bind_text(stmt, 4, record.chromosome_json.c_str(), -1,
                    SQLITE_TRANSIENT);
  ...
  if (rc != SQLITE_DONE) {
    last_error_ = sqlite3_errmsg(db_);
    LOG_ERROR(LogComponent::Database,
              "Failed to insert optimization solution: {}", last_error_);
    return -1;
  }
...
DatabaseManager::OptimizationSolutionRecord
DatabaseManager::getBestOptimizationSolution(int run_id) {
  std::string sql =
      "SELECT id, optimization_run_id, is_best_solution, fitness, "
...
std::vector<DatabaseManager::OptimizationSolutionRecord>
DatabaseManager::getOptimizationSolutions(int run_id) {
...
DatabaseManager::MetricStatistics
DatabaseManager::getMetricStatistics(int simulation_id,
                                     const std::string &metric_type) {
  ...
  std::string sql_median =
      "SELECT value FROM metrics WHERE simulation_id = ? AND metric_type = ? "
...
std::map<std::string, DatabaseManager::MetricStatistics>
DatabaseManager::getAllMetricStatistics(int simulation_id) {
  std::map<std::string, MetricStatistics> all_stats;

  // Get all unique metric types for this simulation
  std::string sql =
      "SELECT DISTINCT metric_type FROM metrics WHERE simulation_id = ?";
  sqlite3_stmt *stmt;
  int rc = sqlite3_prepare_v2(db_, sql.c_str(), -1, &stmt, nullptr);
...
std::vector<DatabaseManager::ComparativeMetrics>
DatabaseManager::getComparativeMetrics(const std::vector<int> &simulation_ids,
                                       const std::string &metric_type) {
...
bool DatabaseManager::insertTrafficSnapshot(
    const TrafficSnapshotRecord &record) {
  std::string sql =
  ...
  if (rc != SQLITE_OK) {
    last_error_ = sqlite3_errmsg(db_);
    LOG_ERROR(LogComponent::Database,
              "Failed to prepare traffic snapshot insert: {}", last_error_);
    return false;
  }
...
bool DatabaseManager::insertTrafficSnapshotsBatch(
    const std::vector<TrafficSnapshotRecord> &records) {
  if (records.empty())
    return true;
  ...
  if (rc != SQLITE_OK) {
    last_error_ = sqlite3_errmsg(db_);
    LOG_ERROR(LogComponent::Database, "Failed to prepare batch insert: {}",
              last_error_);
    executeSQL("ROLLBACK");
    return false;
  }
  ...
  LOG_DEBUG(LogComponent::Database, "Inserted {} traffic snapshots",
            records.size());
  return true;
}

std::vector<DatabaseManager::TrafficSnapshotRecord>
DatabaseManager::getTrafficSnapshots(int64_t since_timestamp) {
  std::vector<TrafficSnapshotRecord> results;
  std::string sql =
      "SELECT id, timestamp, road_id, vehicle_count, queue_length, avg_speed, "
      "flow_rate "
...
std::vector<DatabaseManager::TrafficSnapshotRecord>
DatabaseManager::getTrafficSnapshotsForRoad(int road_id,
                                            int64_t since_timestamp) {
  std::vector<TrafficSnapshotRecord> results;
  std::string sql = "SELECT id, timestamp, road_id, vehicle_count, "
                    "queue_length, avg_speed, flow_rate "
                    "FROM traffic_snapshots WHERE road_id = ? AND timestamp >= "
                    "? ORDER BY timestamp ASC";
...
std::vector<DatabaseManager::TrafficSnapshotRecord>
DatabaseManager::getTrafficSnapshotsRange(int64_t start_time,
                                          int64_t end_time) {
  std::vector<TrafficSnapshotRecord> results;
  std::string sql = "SELECT id, timestamp, road_id, vehicle_count, "
                    "queue_length, avg_speed, flow_rate "
...
int DatabaseManager::deleteTrafficSnapshotsBefore(int64_t timestamp) {
  ...
  LOG_INFO(LogComponent::Database,
           "Deleted {} traffic snapshots before timestamp {}", deleted,
           timestamp);
  return deleted;
}

// Traffic pattern operations
bool DatabaseManager::insertOrUpdateTrafficPattern(
    const TrafficPatternRecord &record) {
  std::string sql =
  ...
  if (rc != SQLITE_OK) {
    last_error_ = sqlite3_errmsg(db_);
    LOG_ERROR(LogComponent::Database,
              "Failed to prepare traffic pattern upsert: {}", last_error_);
    return false;
  }
...
DatabaseManager::TrafficPatternRecord
DatabaseManager::getTrafficPattern(int road_id, int day_of_week,
                                   int time_slot) {
  ...
  std::string sql =
      "SELECT id, road_id, day_of_week, time_slot, avg_vehicle_count, "
      ...
      "FROM traffic_patterns WHERE road_id = ? AND day_of_week = ? AND "
      "time_slot = ?";
...
std::vector<DatabaseManager::TrafficPatternRecord>
DatabaseManager::getTrafficPatterns(int day_of_week, int time_slot) {
...
std::vector<DatabaseManager::TrafficPatternRecord>
DatabaseManager::getTrafficPatternsForRoad(int road_id) {
  std::vector<TrafficPatternRecord> results;
  std::string sql =
      "SELECT id, road_id, day_of_week, time_slot, avg_vehicle_count, "
      "avg_queue_length, avg_speed, avg_flow_rate, min_vehicle_count, "
...
std::vector<DatabaseManager::TrafficPatternRecord>
DatabaseManager::getAllTrafficPatterns() {
  std::vector<TrafficPatternRecord> results;
  std::string sql =
      "SELECT id, road_id, day_of_week, time_slot, avg_vehicle_count, "
      "avg_queue_length, avg_speed, avg_flow_rate, min_vehicle_count, "
...
int DatabaseManager::createProfile(const std::string &name,
                                   const std::string &description) {
  std::string sql =
      "INSERT INTO traffic_profiles (name, description, is_active, created_at) "
      "VALUES (?, ?, 0, ?)";
  ...
  LOG_INFO(LogComponent::Database, "Created traffic profile '{}' with ID: {}",
           name, profile_id);
  return profile_id;
}
...
DatabaseManager::ProfileRecord DatabaseManager::getProfile(int profile_id) {
  ...
  std::string sql = "SELECT id, name, description, is_active, created_at FROM "
                    "traffic_profiles WHERE id = ?";
...
DatabaseManager::ProfileRecord
DatabaseManager::getProfileByName(const std::string &name) {
  ProfileRecord record{};

  std::string sql = "SELECT id, name, description, is_active, created_at FROM "
                    "traffic_profiles WHERE name = ?";
...
std::vector<DatabaseManager::ProfileRecord> DatabaseManager::getAllProfiles() {
  ...
  std::string sql = "SELECT id, name, description, is_active, created_at FROM "
                    "traffic_profiles ORDER BY name";
...
bool DatabaseManager::updateProfile(int profile_id, const std::string &name,
                                    const std::string &description) {
  std::string sql =
      "UPDATE traffic_profiles SET name = ?, description = ? WHERE id = ?";
...
bool DatabaseManager::deleteProfile(int profile_id) {
  ...
  if (rc == SQLITE_DONE) {
    LOG_INFO(LogComponent::Database, "Deleted traffic profile ID: {}",
             profile_id);
    return true;
  }
  return false;
...
bool DatabaseManager::setActiveProfile(int profile_id) {
  ...
  if (rc == SQLITE_DONE) {
    LOG_INFO(LogComponent::Database, "Set active traffic profile ID: {}",
             profile_id);
    return true;
  }
  return false;
...
DatabaseManager::ProfileRecord DatabaseManager::getActiveProfile() {
  ...
  std::string sql = "SELECT id, name, description, is_active, created_at FROM "
                    "traffic_profiles WHERE is_active = 1 LIMIT 1";
...
bool DatabaseManager::saveProfileSpawnRates(
    int profile_id, const std::vector<ProfileSpawnRateRecord> &rates) {
  ...
  std::string sql = "INSERT INTO road_flow_rates (profile_id, road_id, lane, "
                    "vehicles_per_minute, created_at) "
                    "VALUES (?, ?, ?, ?, ?)";
  ...
  LOG_DEBUG(LogComponent::Database, "Saved {} spawn rates for profile {}",
            rates.size(), profile_id);
  return true;
}

std::vector<DatabaseManager::ProfileSpawnRateRecord>
DatabaseManager::getProfileSpawnRates(int profile_id) {
  std::vector<ProfileSpawnRateRecord> results;

  std::string sql =
      "SELECT id, profile_id, road_id, lane, vehicles_per_minute, created_at "
...
bool DatabaseManager::saveProfileTrafficLights(
    int profile_id, const std::vector<ProfileTrafficLightRecord> &lights) {
  ...
  std::string sql = "INSERT INTO profile_traffic_lights (profile_id, road_id, "
                    "lane, green_time, yellow_time, red_time, created_at) "
                    "VALUES (?, ?, ?, ?, ?, ?, ?)";
  ...
  LOG_DEBUG(LogComponent::Database, "Saved {} traffic lights for profile {}",
            lights.size(), profile_id);
  return true;
}

std::vector<DatabaseManager::ProfileTrafficLightRecord>
DatabaseManager::getProfileTrafficLights(int profile_id) {
  std::vector<ProfileTrafficLightRecord> results;

  std::string sql =
      "SELECT id, profile_id, road_id, lane, green_time, yellow_time, "
      "red_time, created_at "
