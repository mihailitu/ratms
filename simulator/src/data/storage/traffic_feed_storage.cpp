...
bool TrafficFeedStorage::recordFeedSnapshot(
    const simulator::TrafficFeedSnapshot &snapshot) {
  ...
  if (sqlite3_exec(db_->getConnection(), "BEGIN TRANSACTION", nullptr, nullptr,
                   &err_msg) != SQLITE_OK) {
    LOG_ERROR(LogComponent::Database, "Failed to start transaction: {}",
  ...
  if (rc != SQLITE_OK) {
    ...
    sqlite3_exec(db_->getConnection(), "ROLLBACK", nullptr, nullptr, nullptr);
    return false;
  }
  ...
  for (const auto &entry : snapshot.entries) {
    ...
    sqlite3_bind_text(stmt, 6, snapshot.source.c_str(), -1, SQLITE_TRANSIENT);
  ...
  // Commit transaction
  if (sqlite3_exec(db_->getConnection(), "COMMIT", nullptr, nullptr,
                   &err_msg) != SQLITE_OK) {
    LOG_ERROR(LogComponent::Database, "Failed to commit transaction: {}",
              err_msg ? err_msg : "Unknown error");
    if (err_msg)
      sqlite3_free(err_msg);
    sqlite3_exec(db_->getConnection(), "ROLLBACK", nullptr, nullptr, nullptr);
  ...
  LOG_TRACE(LogComponent::Database,
            "Recorded {}/{} feed entries from source '{}'", successCount,
            snapshot.entries.size(), snapshot.source);

  return successCount == static_cast<int>(snapshot.entries.size());
}

bool TrafficFeedStorage::recordFeedEntry(
    const simulator::TrafficFeedEntry &entry, const std::string &source) {
  std::lock_guard<std::mutex> lock(mutex_);
  return insertEntry(entry, source);
}

bool TrafficFeedStorage::insertEntry(const simulator::TrafficFeedEntry &entry,
...
std::vector<simulator::TrafficFeedEntry>
TrafficFeedStorage::getEntries(int64_t startTime, int64_t endTime) {
  std::lock_guard<std::mutex> lock(mutex_);
  ...
  if (rc != SQLITE_OK) {
    LOG_ERROR(LogComponent::Database,
              "Failed to prepare feed entries query: {}",
              sqlite3_errmsg(db_->getConnection()));
    return entries;
  }
...
std::vector<simulator::TrafficFeedEntry>
TrafficFeedStorage::getEntriesForRoad(int roadId, int64_t startTime,
                                      int64_t endTime) {
  std::lock_guard<std::mutex> lock(mutex_);
  std::vector<simulator::TrafficFeedEntry> entries;
...
int TrafficFeedStorage::getEntryCount(int64_t startTime, int64_t endTime) {
  ...
  sqlite3_stmt *stmt = nullptr;
  int rc =
      sqlite3_prepare_v2(db_->getConnection(), sql.c_str(), -1, &stmt, nullptr);
...
std::string TrafficFeedStorage::exportToCSV(int64_t startTime,
                                            int64_t endTime) {
  auto entries = getEntries(startTime, endTime);

  std::ostringstream csv;
  csv << "timestamp,road_id,expected_vehicle_count,expected_avg_speed,"
         "confidence\n";

  for (const auto &entry : entries) {
    csv << entry.timestamp << "," << entry.roadId << ","
        << entry.expectedVehicleCount << "," << entry.expectedAvgSpeed << ","
        << entry.confidence << "\n";
  }
...
std::string TrafficFeedStorage::exportToJSON(int64_t startTime,
                                             int64_t endTime) {
  auto entries = getEntries(startTime, endTime);
...
int TrafficFeedStorage::pruneOldEntries(int days) {
  ...
  LOG_INFO(LogComponent::Database, "Pruned {} feed entries older than {} days",
           deleted, days);
...
TrafficFeedStorage::FeedStats TrafficFeedStorage::getStats() {
  ...
  // Total entries
  {
    ...
    sqlite3_stmt *stmt = nullptr;
    if (sqlite3_prepare_v2(db_->getConnection(), sql, -1, &stmt, nullptr) ==
        SQLITE_OK) {
      if (sqlite3_step(stmt) == SQLITE_ROW) {
  ...
  {
    const char *sql =
        "SELECT COUNT(DISTINCT road_id) FROM traffic_feed_entries";
    sqlite3_stmt *stmt = nullptr;
    if (sqlite3_prepare_v2(db_->getConnection(), sql, -1, &stmt, nullptr) ==
        SQLITE_OK) {
      if (sqlite3_step(stmt) == SQLITE_ROW) {
  ...
  {
    const char *sql =
        "SELECT MIN(timestamp), MAX(timestamp) FROM traffic_feed_entries";
    sqlite3_stmt *stmt = nullptr;
    if (sqlite3_prepare_v2(db_->getConnection(), sql, -1, &stmt, nullptr) ==
        SQLITE_OK) {
      if (sqlite3_step(stmt) == SQLITE_ROW) {
  ...
  {
    const char *sql =
        "SELECT source, COUNT(*) FROM traffic_feed_entries GROUP BY source";
    sqlite3_stmt *stmt = nullptr;
    if (sqlite3_prepare_v2(db_->getConnection(), sql, -1, &stmt, nullptr) ==
        SQLITE_OK) {
