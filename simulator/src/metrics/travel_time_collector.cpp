...
int TravelTimeCollector::addODPair(int originRoadId, int destinationRoadId,
  ...
  for (const auto &[id, pair] : odPairs_) {
    if (pair.originRoadId == originRoadId &&
  ...
  pair.name = name.empty() ? "Road " + std::to_string(originRoadId) + " -> " +
                                 std::to_string(destinationRoadId)
                           : name;
  ...
  LOG_INFO(LogComponent::Core, "Added O-D pair {}: {} -> {} ({})", pair.id,
           originRoadId, destinationRoadId, pair.name);
...
void TravelTimeCollector::removeODPair(int odPairId) {
  ...
  for (auto trackIt = trackedVehicles_.begin();
       trackIt != trackedVehicles_.end();) {
...
void TravelTimeCollector::update(const simulator::Simulator::CityMap &cityMap,
                                 double /*dt*/) {
  std::lock_guard<std::mutex> lock(mutex_);
    ...
    for (unsigned lane = 0; lane < vehicles.size(); ++lane) {
      for (const auto &vehicle : vehicles[lane]) {
        ...
        if (trackedIt != trackedVehicles_.end()) {
          ...
          if (roadId ==
              static_cast<unsigned long>(trackedIt->second.destinationRoadId)) {
          ...
          for (int odPairId : odPairIds) {
            ...
            startTracking(vehicleId, odPairId, pair.originRoadId,
                          pair.destinationRoadId);
          }
        }
      }
    }
  ...
  for (auto it = trackedVehicles_.begin(); it != trackedVehicles_.end();) {
    if (vehiclesSeenThisStep_.find(it->first) == vehiclesSeenThisStep_.end()) {
      LOG_DEBUG(LogComponent::Core,
                "Vehicle {} left network before reaching destination",
                it->first);
...
std::vector<TravelTimeSample>
TravelTimeCollector::getRecentSamples(int odPairId, int limit) const {
  std::lock_guard<std::mutex> lock(mutex_);

  std::vector<TravelTimeSample> result;

  // Return from pending samples first
  for (auto it = pendingSamples_.rbegin();
       it != pendingSamples_.rend() &&
...
void TravelTimeCollector::flush() {
  ...
  // TODO: Batch insert to database when DatabaseManager supports travel time
  // tables For now, just update in-memory stats

  for (const auto &sample : pendingSamples_) {
    LOG_DEBUG(LogComponent::Database,
              "Would save travel time sample: O-D {} vehicle {} time {:.2f}s",
...
void TravelTimeCollector::startTracking(int vehicleId, int odPairId,
  ...
  LOG_TRACE(LogComponent::Core, "Started tracking vehicle {} for O-D pair {}",
            vehicleId, odPairId);
}
...
void TravelTimeCollector::recordCompletion(const TrackedVehicle &vehicle) {
  ...
  sample.startTime = std::chrono::duration_cast<std::chrono::seconds>(
                         vehicle.startTime.time_since_epoch())
                         .count();
...
void TravelTimeCollector::updateStats(int odPairId) {
  ...
  LOG_DEBUG(LogComponent::Core,
            "Updated stats for O-D {}: avg={:.2f}s min={:.2f}s max={:.2f}s "
            "p95={:.2f}s samples={}",
            odPairId, stats.avgTime, stats.minTime, stats.maxTime,
            stats.p95Time, stats.sampleCount);
}
...
bool TravelTimeCollector::isVehicleOnDestination(
    int vehicleId, int destinationRoadId,
    const simulator::Simulator::CityMap &cityMap) const {
  auto roadIt = cityMap.find(destinationRoadId);
  if (roadIt == cityMap.end())
    return false;
