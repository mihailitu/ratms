...
double SimulationMetrics::getFitness() const {
    ...
    if (sampleCount < 10) {
        ...
        double vehiclePenalty = totalVehicles * 0.1;      // Slight penalty for active vehicles
    ...
    double exitRatio = (totalVehicles > 0) ? (vehiclesExited / totalVehicles) : 0.0;
...
void MetricsCollector::collectMetrics(const Simulator::CityMap& cityMap, double dt) {
    ...
    metrics_.maxQueueLength = std::max(metrics_.maxQueueLength, currentQueueLength);
...
size_t FitnessEvaluator::countTrafficLights(const std::vector<Road>& roadNetwork) {
    size_t count = 0;
...
void FitnessEvaluator::applyChromosome(const Chromosome& chromosome, Simulator::CityMap& cityMap) {
    size_t geneIdx = 0;
    ...
    for (auto& roadPair : cityMap) {
        ...
        // In a production version, we'd add a method to Road to update traffic light timings
...
double FitnessEvaluator::evaluate(const Chromosome& chromosome, std::vector<Road>& roadNetwork) {
    ...
    for (Road& road : roadNetwork) {
        // Note: Since we can't directly modify traffic lights after Road construction,
        // we need to recreate roads with new timings
        // This is a limitation of the current Road class design

        // For this prototype, we'll add roads as-is and track metrics
        // A production version would require refactoring Road to allow traffic light updates
