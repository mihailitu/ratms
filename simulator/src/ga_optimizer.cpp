...
/**
 * @brief runBaselineSimulation - Run simulation with fixed traffic light
 * timings
 */
SimulationMetrics runBaselineSimulation(std::vector<Road> &roadNetwork,
                                        int steps, double dt) {
  Simulator sim;
  sim.cityMap.clear();
...
int main(int argc, char *argv[]) {
  ...
  ratms::Logger::init();
  LOG_INFO(LogComponent::General, "GA Optimizer starting");
  for (int i = 1; i < argc; ++i) {
    ...
    if (arg == "--pop" && i + 1 < argc) {
      ...
      std::cout
          << "  --steps N    Simulation steps per evaluation (default: 1000)\n";
      std::cout << "  --help       Show this help message\n";
      return 0;
    }
  }

  LOG_INFO(LogComponent::Optimization, "Config: pop={}, gen={}, steps={}",
           gaParams.populationSize, gaParams.generations, simulationSteps);
  ...
  LOG_INFO(LogComponent::Simulation, "Test network: {} roads, {} traffic lights",
           testNetwork.size(), totalTrafficLights);
  ...
  LOG_INFO(LogComponent::Simulation, "Running baseline simulation");
  SimulationMetrics baselineMetrics;
  double baselineFitness;
  {
    TIMED_SCOPE(LogComponent::Simulation, "baseline_simulation");
    std::vector<Road> baselineNetwork = createTestIntersection();
    baselineMetrics = runBaselineSimulation(baselineNetwork, simulationSteps, dt);
    baselineFitness = baselineMetrics.getFitness();
  }
  LOG_INFO(LogComponent::Simulation, "Baseline fitness: {:.4f}", baselineFitness);

  std::cout << "Baseline Results:\n";
  std::cout << "  Average Queue Length: " << baselineMetrics.averageQueueLength
            << " vehicles\n";
  ...
  auto fitnessFunc = [&evaluator,
                      &testNetwork](const Chromosome &chromosome) -> double {
  ...
  LOG_INFO(LogComponent::Optimization, "Starting GA evolution");
  auto startTime = std::chrono::high_resolution_clock::now();

  Chromosome bestSolution;
  {
    TIMED_SCOPE(LogComponent::Optimization, "ga_evolution");
    bestSolution = ga.evolve();
  }
  ...
  double improvement =
      ((baselineFitness - bestSolution.fitness) / baselineFitness) * 100.0;

  LOG_INFO(LogComponent::Optimization, "Optimization complete: baseline={:.4f}, best={:.4f}, improvement={:.2f}%",
           baselineFitness, bestSolution.fitness, improvement);
  ...
  for (size_t i = 0; i < bestSolution.genes.size(); ++i) {
    std::cout << "  Light " << i
              << ": Green=" << bestSolution.genes[i].greenTime
